...
Авторея uses code signing so that Windows users can verify that release files
...
- Release binaries are built from the public
  [source repository](https://github.com/ryzhkevichpavel-del/avtoreya).
- The automated Windows build uses
  [GitHub Actions](https://github.com/ryzhkevichpavel-del/avtoreya/actions).
- Product names and version numbers in the executable and installer must match
  the release version.
- Only official Авторея release files built from this repository may be signed.
- Signed releases are published on
  [GitHub Releases](https://github.com/ryzhkevichpavel-del/avtoreya/releases).
